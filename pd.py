...
def fetch_all_videos(connection):
    try:
        ...
        cursor.execute("SELECT vname, genre, tags, location, pdf_location FROM tables")
...
def recommend_videos(input_video_name, limit=3):
    ...
    df['features'] = df['vname'] + " " + df['genre'] + " " + df['tags']
...
def index():
    ...
    cursor = connection.cursor(dictionary=True)
    cursor.execute("SELECT vname, genre, tags, location, pdf_location FROM tables ORDER BY id DESC LIMIT 5")
    videos = cursor.fetchall()
    cursor.close()
